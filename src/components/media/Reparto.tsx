...
export default async function Reparto({
    ...
    // Fetch cast and crew data for the specified movie or series
    const start: Reparto = await fetchRepartoMovie(type, id);

    // Find the director or the first crew member
    const director =
        start.crew.find(
            (miembro) =>
